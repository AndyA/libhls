...
static void format_record(jd_var *lb, jd_var *syn, jd_var *rec) {
  if (!rec) return;
...
static void format_list(jd_var *lb, jd_var *syn, jd_var *list) {
  for (unsigned i = 0; i < jd_count(list); i++)
    format_record(lb, syn, jd_get_idx(list, i));
}
...
jd_var *hls_m3u8_format(jd_var *out, jd_var *m3u8) {
  scope {
    ...
    format_record(lb, syn, jd_get_ks(m3u8, "meta", 0));
    format_list(lb, syn, jd_get_ks(m3u8, "vpl", 0));
    format_list(lb, syn, jd_get_ks(m3u8, "seg", 0));
